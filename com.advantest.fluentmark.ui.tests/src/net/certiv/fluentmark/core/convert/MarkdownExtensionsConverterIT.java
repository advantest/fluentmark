...
""";
	...
	@ParameterizedTest
	@EnumSource(names = { "FLEXMARK" }) // ignore pandoc, since it does not remove backslashes in URLs and our focus is flexmark anyway
...
""";
		...
		assertTrue(result.matches("<h3[^<>]*>Implementation details</h3>\\s*"
				+ "<p[^<>]*>Here is my\\s+"
				+ "<a\\s+href=\\\"\\.\\./\\.\\./path/to/ImportantClassOrInterface\\.java#doSomething\\(int,boolean,Character%5B%5D,List%3CMap%3CK,V%3E%3E\\)\\\"[^<>]*>important\\s+method</a>\\s+"
				+ "and here is a\\s+"
